...
const schema = z.object({
  ...
  // Hybrid execution boundary: Phases A-C are immutable local-only stages.
  SWARMX_PHASE_ABC_EXECUTION: z.literal("local").default("local"),
  SWARMX_PHASE_D_EXECUTION: z.enum(["local", "aws_fargate"]).default("local"),
  ...
  DATABASE_URL_UNPOOLED: z.preprocess((value) => value === "" ? undefined : value, z.string().url().optional()),
  FAL_KEY: z.preprocess((value) => value === "" ? undefined : value, z.string().min(1).optional()),
  ...
  SWARMX_VIDEO_MAX_CONCURRENT_JOBS: z.preprocess((val) => val ?? process.env["MAX_CONCURRENT_JOBS"] ?? process.env["VIDEO_MAX_CONCURRENT_JOBS"], positiveInt.default(1)),
